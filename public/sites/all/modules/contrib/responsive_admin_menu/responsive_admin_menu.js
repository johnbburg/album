...
Drupal.behaviors.adminMenu = {
  attach: function (context, settings) {
    // Initialize settings.
    settings.responsive_admin_menu = $.extend({
      suppress: false,
      margin_top: false,
      ...
      hash: 0,
      replacements: {}
    }, settings.responsive_admin_menu || {});
    // Check whether administration menu should be suppressed.
    if (settings.responsive_admin_menu.suppress) {
      return;
    }
    var $adminMenu = $('#admin-menu:not(.admin-menu-processed)', context);
    // Client-side caching; if administration menu is not in the output, it is
    // fetched from the server and cached in the browser.
    if (!$adminMenu.length && settings.responsive_admin_menu.hash) {
      Drupal.admin.getCache(settings.responsive_admin_menu.hash, function (response) {
...
Drupal.behaviors.adminMenuCollapseModules = {
  attach: function (context, settings) {
    if (settings.responsive_admin_menu.tweak_modules) {
...
Drupal.behaviors.adminMenuCollapsePermissions = {
  attach: function (context, settings) {
    if (settings.responsive_admin_menu.tweak_permissions) {
...
Drupal.behaviors.adminMenuMarginTop = {
  attach: function (context, settings) {
    if (!settings.responsive_admin_menu.suppress && settings.responsive_admin_menu.margin_top) {
...
Drupal.admin.getCache = function (hash, onSuccess) {
  ...
  $.ajax({
    ...
    url: Drupal.settings.responsive_admin_menu.basePath.replace(/responsive_admin_menu/, 'js/responsive_admin_menu/cache/' + hash),
    success: onSuccess,
...
Drupal.admin.behaviors.positionFixed = function (context, settings, $adminMenu) {
  if (settings.responsive_admin_menu.position_fixed) {
...
Drupal.admin.behaviors.pageTabs = function (context, settings, $adminMenu) {
  if (settings.responsive_admin_menu.tweak_tabs) {
...
Drupal.admin.behaviors.replacements = function (context, settings, $adminMenu) {
  for (var item in settings.responsive_admin_menu.replacements) {
    $(item, $adminMenu).html(settings.responsive_admin_menu.replacements[item]);
  }
};
...
Drupal.admin.behaviors.destination = function (context, settings, $adminMenu) {
  if (settings.responsive_admin_menu.destination) {
    $('a.admin-menu-destination', $adminMenu).each(function() {
      this.search += (!this.search.length ? '?' : '&') + Drupal.settings.responsive_admin_menu.destination;
    });
  }
};
